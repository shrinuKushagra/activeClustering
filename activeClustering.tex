...
A more interesting but subtle challenge is revealed by noting that the `clustering with query' model can be contrasted to the usual unsupervised clustering. Assume that the oracle has the optimal $k$-means clustering in its mind. Finding the $k$-means solution without the help of the oracle is generaly computationally hard. Therefore, one may ask whether under $\gamma$-margin assumption one can provide a polynomial time algorithm that does not make any queries. In the next section we show a negative answer: for $\gamma < 1.5$ finding the solution to $k$-means clustering is computationally hard. This together with Theorem \ref{thm:steinerQueryPositive} and \ref{thm:steinerQueryPositiveComplexity} shows that for $1<\gamma<1.5$, having access to queries makes the computationally hard problem of $k$-means clustering efficiently solvable\footnote{To be precise, note that the algorithm used for clustering with queries is probabilistic. However, the lower bound that we provide is for deterministic algorithms. However, this means a lower bound for randomized algorithms as well unless $BPP\neq P$}.
...
One can also consider a weighted version of the problem \cite{vattani2009hardness} where every point $x$ has a weight $w(x)$. 

$$\sum\limits_{C_i}\frac{1}{\sum_{x \in C_i}w(x)} \sum_{x, y \in {C_i \choose 2}} w(x)w(y)d^2(x, y).$$ 

Vattani showed \cite{vattani2009hardness} that finding the optimal solution to the weighted $k$-means objective is NP-Hard even in Euclidean plane. We will also use similar ideas and reduction technique to get our lower bound. However, Vattani's proof does not imply hardness under $\gamma$-margin assumption. Therefore, we have to modify the proof by setting up the construction more carefully.
...
\begin{definition}[Good point for a cluster \cite{vattani2009hardness}]
A cluster $C$ is good for a point $x \not\in C$ if adding $z$ to $C$ increases cost by exactly $\frac{2ww'}{2w+w'}h^2$ 
\end{definition}

A row grouped in a $A$ clustering costs $(6n+3)w-\alpha$ while a row in $B$ clustering costs $(6n+3)w$. Also, the following simple observations (also in \cite{vattani2009hardness}) can be made. 
\begin{itemize}[nolistsep,noitemsep]
