...
For a clustering $C^*_{\mc X}=\{ C^*_1, \ldots C^*_k\}$, a $C^*$ oracle is a
a function that provides answers to queries according to that clustering. One can think of such an oracle as a suer that has some idea about its desired clustering, enough to answer the algorithm's queries. The clustering algorithm then tries to recover $C^*$ by querying a $C^*$ oracle. The following notions of query are natural for clustering tasks.
...
\begin{lemma}
For large enough $w = poly(l, n)$, any non-nice $k$-clustering of $H_{l, n}$ costs at least $(L + \frac{7}{8}w)$.%$  (L_1 + L_2 - l\alpha + w) > (L + \frac{7}{8}w)$.
\end{lemma}
