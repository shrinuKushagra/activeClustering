...
\begin{abstract}
We provide a framework for clustering where the learner is allowed to ask \emph{same-cluster} queries from an oracle (e.g., a domain expert). In this type of query, the oracle indicates whether two given instances belong to the same cluster or not. We study the query and computaitonal complexity in this framework. In order to do that, we consider a setting where 
